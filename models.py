...
class AutoEncoderNoShortcuts(nn.Module):
    """
    Input: [batch_size, channels, 157, 189, 156] (recommended)
    ...
    * embedding_dim: the embedding dimensions of the vit block
    ...
    def __init__(self,
        ...
        self.consecutive_transpose_convnets = DecodeConsecutiveConvNets(in_channel=conv_in_channels,
                                                                        out_channel=conv_out_channels,
                                                                        kernel_size=kernel_size,
                                                                        padding=True)
    ...
    def forward(self, x):
        ...
        for i, j in cache.items():
            cache[i] = torch.zeros_like(j).detach()

        x = self.consecutive_transpose_convnets(
            x, cache)
...
class AutoEncoderWithoutShortcuts(nn.Module):
    ...
    def __init__(self,
                 ...
                 mlp_dropout: float = .1,
                 embedding_dropout: float = .1,
                 final_latent_space_dim: int = 4096):
        super().__init__()
        assert conv_out_channels == 64 or conv_out_channels == 256, "Unsupportable Channels"
...
class ViTEncoder(nn.Module):
    def __init__(self,
        ...
        self.transformer_encoder = nn.Sequential(*[TransformerEncoder(embedding_dim=embedding_dim,
                                                                      ...
                                                                      mlp_size=mlp_size,
                                                                      mlp_dropout=mlp_dropout,
                                                                      attention_dropout=attention_dropout) for _
    ...
    def forward(self, x):
        patch_embeded = self.patch_embedding(x).clone()
...
class Consecutive3DConvLayerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = nn.Conv3d(
            32, 64, (kernel_size, kernel_size, kernel_size), padding=1, stride=2)
    ...
    def forward(self, x):
        ...
        if self.padding:
            x = _pad_3D_image_patches_with_channel(
                x, [x.shape[0], x.shape[1], 158, 189, 156])
            # cache["first_padding"] = x.clone()
        x = self.conv1(x)
        # cache["conv1"] = x.clone()
        if self.padding:
            x = _pad_3D_image_patches_with_channel(
                x, [x.shape[0], x.shape[1], 64, 64, 64])
        ...
        return x.reshape(x.shape[0], 1, out_shape, -1)
...
class InitialResidualNet(nn.Module):
    def __init__(self,
        ...
        self.unflatten = nn.Unflatten(
            dim=2, unflattened_size=(sqrt_num_patches, sqrt_num_patches))
    ...
    def forward(self, latent_space_vec):
        ...
        #     f"[ERROR] Dimensions Mismatch: {output.shape} != {patch_and_pos_embedded.shape}"
...
class DecodeConsecutiveConvNets(nn.Module):
    ...
    def forward(self, x,):
        ...
        if self.padding:
            x = _unpad_3D_image_patches_with_channel(
                x, (52, 63, 52)) #+ cache_from_convnets["conv1"]
...
def _unpad_3D_image_patches_with_channel(padded_img, original_size):
    ...
    # use ellipsis to keep the first two dimensions (batch and channel) unchanged
    unpadded = padded_img[..., slices[0], slices[1], slices[2]]
    return unpadded
